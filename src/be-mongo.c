...
bool be_mongo_check_acl_topics_array(const bson_iter_t* topics, const char* req_topic, const char* clientid,
                                     const char* username);
...
void* be_mongo_init()
{
    ...
    conf->user_password_prop =
        strdup(be_mongo_get_option("mongo_user_password_prop", "mongo_location_password", "password"));
    conf->user_superuser_prop =
        strdup(be_mongo_get_option("mongo_user_superuser_prop", "mongo_location_superuser", "superuser"));
    conf->user_topics_prop = strdup(be_mongo_get_option("mongo_user_topics_prop", NULL, "topics"));
    conf->user_topiclist_fk_prop =
        strdup(be_mongo_get_option("mongo_user_topiclist_fk_prop", "mongo_location_topic", "topics"));
    conf->topiclist_key_prop =
        strdup(be_mongo_get_option("mongo_topiclist_key_prop", "mongo_location_superuser", "_id"));
    conf->topiclist_topics_prop =
        strdup(be_mongo_get_option("mongo_topiclist_topics_prop", "mongo_location_topic", "topics"));
...
mongoc_uri_t* be_mongo_new_uri_from_options()
{
    ...
    else if (host || port || user || password || authSource)
    {
        ...
        uri = mongoc_uri_new_for_host_port(host ? host : "localhost", (port && atoi(port)) ? atoi(port) : 27017);
...
int be_mongo_aclcheck(void* conf, const char* clientid, const char* username, const char* topic, int acc)
{
    ...
    if (!match && (topic_lookup_oid != NULL || topic_lookup_int64 != 0 || topic_lookup_utf8 != NULL))
    {
        ...
        if (!mongoc_cursor_error(cursor, &error) && mongoc_cursor_next(cursor, &doc))
        {
            ...
            {
                _log(LOG_NOTICE, "[mongo] ACL check error - no topic list found for user (%s) in collection (%s)",
                     username, handle->topiclist_coll);
            }
        }
